...
function showDeleteForm (event) {
    ...
    const userUsername = document.getElementById('delete-user-username')
    ...
    userUsername.value = tableRow.querySelector('td:nth-child(2)').textContent;
...
function showEditForm (event) {
    ...
    const userUsername = document.getElementById('edited-user-username')
...
editUserForm.addEventListener('submit', event => {
    ...
    const username = document.getElementById('edited-user-username').value;
    const userData = {
    ...
    fetch(URL, options)
        .then(response => {
            if (response.ok) {
                ...
                showAlert(`The user with username '${username}' was edited successfully`, 'alert-success', 'user-added-alert')
                refreshTable()
            } else {
    ...
    fetch(URL, options)
        .then(response => {
            if (response.ok) {
                ...
                showAlert(`The user with username '${username}' was added successfully`, 'alert-success', 'user-added-alert')
                refreshTable()
            } else {
                response.json().then(json => {
